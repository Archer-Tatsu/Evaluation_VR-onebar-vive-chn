...
handles=ResetDirData(handles);
 ...
 if(~isempty(varargin{2}))
     handles.DirData.SelectedList=varargin{2};
     handles.DirData.NumMediaFiles=size(handles.DirData.SelectedList,2);
            ...
            otherwise
                %Check if there is a handler for this extension
                    ...
                    if(2==exist(fname,'file'))
                        ...
                        errordlg('包含不支持的文件类型','Unsupported files','modal');
                        return;
                    end
        end %switch(lower(ext))
        temppath=fullfile(Tok{i},FilesInDir{j});
        if(sum(sum(isChinese(temppath)))>0)
            errordlg('路径包含双字节字符','Unsupported files','modal');
            return;
        end
        handles.DirData.SelectedList{LastElement+j}=temppath;
            ...
            otherwise
                %Check if there is a handler for this extension
                    ...
                    if(2==exist(fname,'file'))
                        ...
                        errordlg('不支持选择的文件类型','Unsupported files','modal');
                        return;
                    end
            end %switch(lower(ext))
            
            temppath=fullfile(handles.DirData.CurrPath,handles.DirData.FileList{handles.DirData.FilesSelInDir(i)});
            if(sum(sum(isChinese(temppath)))>0)
                errordlg('路径包含双字节字符','Unsupported files','modal');
                return;
            end
            handles.DirData.SelectedList{LastElement+i}=temppath;
                ...
                otherwise
                    %Check if there is a handler for this extension
                        ...
                        if(2==exist(fname,'file'))
                            ...
                            errordlg('不支持的文件格式','Unsupported files','modal');
                            return;
                        end
            end %switch(lower(ext))
...
[OutputFile, OutputPath] = uiputfile(Filterspec,'请指定输出文件.');
...
if((isfield(handles.DirData,'SelectedList')))
    ...
    if((handles.DirData.NumListFiles>0) && ...
       ((~isempty(find(handles.DirData.RawFileLocations, 1)) ) || (handles.DirData.NumMediaFiles > 0))...
        ...
        error('列表文件不应与视频文件混合添加','Mixed File types selected','modal');
    end

    if((1==handles.DirData.SaveFileList) && ...
       ((handles.DirData.NumListFiles > 0))...
      )
        error('列表中包含非视频文件，无法保存到列表','Filelist save not possible','modal');
    end

    if((1==handles.DirData.SaveFileList) && isempty(handles.DirData.OutputFilename))
        error('未指定输出文件','No output file specified','modal');
    end
            ...
            for j=1:NumOfFilesInList
                ...
                if(2==exist(char(filelist(j)),'file'))
                    ...
                    if(~isempty(fpath))
                        ...
                        tmp=char(filelist(j));
                        if(sum(sum(isChinese(tmp)))>0)
                            warndlg(['路径' tmp '包含双字节字符，将被跳过'],'警告','modal');
                            uiwait;
                            continue;
                        end
                    else
                        ...
                        if(isempty(tmp))
                            ...
                            warndlg([filelist(j) '不存在，将被跳过。'],'警告','modal');
                        end
                    end
    ...
    if(1==handles.DirData.SaveFileList)
        ...
        switch tmp
            ...
            case {'csv','txt'}
                  ...
                  if(-1==fid)
                        error('无法打开待写文件','Could not open file','modal');
                  else
            ...
            otherwise
                  error('不支持的文件格式','Unsupported file format','modal');
    ...
    Output=[];
    error('请选择一个文件','No File specified','modal');
...
function [FileList]=GenFileList(handles,TempDir,CurrDir)
    ...
    switch(handles.FileFilterToApply)
        ...
        case 2
            ...
            FormatFile = which('SupVFormats.csv');
