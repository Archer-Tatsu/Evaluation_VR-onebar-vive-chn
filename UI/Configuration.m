...
handles.Player=[];
[filename, pathname] = uigetfile('*.exe','选择VR全景视频播放器...');
if(filename==0)
    errordlg('无效的播放器路径','输入无效','modal');
    error('无效的播放器路径');
    return;
else
...
try
    ...
    errordlg(RetErr.message,'错误','modal');
    uiwait;
end
%Activate start
...
FileDetails=SelectSource(handles.MediaType,handles.SubTestData.RefFiles);
if((isfield(FileDetails,'Format')) && (~isempty(FileDetails.Format)))
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
FileDetails=SelectSource(handles.MediaType,handles.SubTestData.TestFiles);
if((isfield(FileDetails,'Format')) && (~isempty(FileDetails.Format)))
...
[OutputFile, OutputPath] = uiputfile(Filterspec,'请指定原始主观分数输出文件.');
if((isnumeric(OutputFile)) &&(0==OutputFile))
else
...
[OutputFile, OutputPath] = uiputfile(Filterspec,'请指定Z分数输出文件.');
if((isnumeric(OutputFile)) &&(0==OutputFile))
else
